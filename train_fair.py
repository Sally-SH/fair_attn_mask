...
def main():
    ...
    dropout_rate = 0.1
    mask_output_dim = 2
    fair_output_dim = args.num_verb if args.dataset=='imsitu' else args.num_att-1 # gender 제외 target 수
...
def train_imsitu(args, epoch, fair_model, mask_model, mask_criterion, orig_criterion, train_loader, optimizer, scheduler,\
    train_logger, logging=True):
    fair_model.train()
    nProcessed = 0
    ...
    for batch_idx, (images, targets, genders, image_ids) in enumerate(t):
        ...
        mask_preds, _ = fair_model(masked_images) # mask_preds : [B, 211]
        #orig_preds, _ = fair_model(images.cuda()) # [B, 211]
        ...
        preds_max = mask_preds.max(1, keepdim=False)[1] # [32,]

        # save the exact preds (binary)
        tensor = torch.tensor((), dtype=torch.float64)
        #preds_exact = tensor.new_zeros(orig_preds.size())
        preds_exact = tensor.new_zeros(mask_preds.size()) # [32, 1]
        #pdb.set_trace()
...
def test_imsitu(args, epoch, fair_model, mask_model, mask_criterion, orig_criterion, val_loader, val_logger, logging=True):
...
def train_celeba(args, epoch, fair_model, mask_model, mask_criterion, orig_criterion, train_loader, optimizer, scheduler,\
    train_logger, logging=True):
    fair_model.train()
    nProcessed = 0
    nTrain = len(train_loader.dataset) # number of images
    ...
    for batch_idx, (images, targets) in enumerate(t):
        ...
        genders = zero.scatter_(1, genders.view(genders.size(0),1).long(), 1) # [16, 2]
        # one hot encoded target
        zero = torch.zeros(targets.size(0), 2).cuda()
        targets = zero.scatter_(1, targets.view(targets.size(0),1).long(), 1) # [16, 2]
        ...
        # set mini-batch dataset
        masked_images = mask_image(args, mask_model, images) # [32, 3, 256, 256]
        masked_images = masked_images.cuda()
        targets = targets.cuda() # [32, 2]

        # forward, Backward and Optimize
        mask_preds, _ = fair_model(masked_images) # [32,1]
        #orig_preds, _ = fair_model(images.cuda())
        ...
        #orig_preds = F.softmax(orig_preds, dim=1)
        #preds_max = orig_preds.max(1, keepdim=True)[1]
        mask_preds = F.softmax(mask_preds, dim=1) # [32, 1]
        preds_max = mask_preds.max(1, keepdim=True)[1]
        ...
        #res.append((image_ids, orig_preds.detach().cpu(), targets.detach().cpu(), genders, preds_exact))
        res.append((mask_preds.detach().cpu(), targets.detach().cpu(), genders, preds_exact))
    ...
    total_preds_exact = torch.cat([entry[3] for entry in res], 0)
